...
当前为 OADIN 预览版 v0.4.0，更多功能和稳定性正在不断完善过程中。欢迎就发现的缺陷提交 Issues。

当前版本支持 chat、embed、text-to-image 服务，下层支持 ollama 和 openvino model server。更多服务如视频、音频相关，以及其他 AI 引擎，敬请
期待正在开发的后续版本。

详细中文文档请参见[此处](https://intel.github.io/oadin/index.html)

## OADIN 的功能

OADIN（AIPC Open Gateway）的目标是解耦 AI PC 上的 AI 应用与它所依赖的 AI 服务。它旨在为开发者提供一个
...
OADIN 提供了一个控制面板，您可以通过浏览器访问它。Control Panel 提供了图形化界面来管理OADIN的服务、模型和配置。
...
确保前端页面已构建完成后，在命令行中输入以下命令来启动 OADIN：

```sh
# 前台启动OADIN（推荐用于开发调试）
oadin server start -v
...
在 About OADIN 界面，您可以查看现有的服务，及其支持下载的本地模型列表和支持认证的远程模型列表。
...
OADIN 包含前端 Control Panel 和后端命令行工具两个部分。为了确保完整的功能，需要按照以下顺序进行构建：

### 第一步：构建前端 Control Panel

在构建 OADIN 命令行工具之前，**必须先构建前端 Control Panel**，因为 OADIN 会将前端资源嵌入到可执行文件中。
...
4. **构建 OADIN 可执行文件**
   ...
   **Linux/macOS:**
   ```sh
   CGO_ENABLED=1 go build -o oadin -ldflags="-s -w" cmd/cli/main.go
   ```
...
- **前端构建是必需的**：如果没有先构建前端，OADIN 的 Control Panel 功能将无法正常工作
- **CGO 依赖**：OADIN 需要启用 CGO，确保您的环境支持 C 编译器
...
echo "Building OADIN - Complete Build Process"
...
# Step 2: Build OADIN
echo "Step 2: Building OADIN command line tool..."
CGO_ENABLED=1 go build -o oadin -ldflags="-s -w" cmd/cli/main.go

echo "Build completed successfully!"
echo "You can now run: ./oadin server start"
```
...
echo Build completed successfully!
echo You can now run: oadin.exe server start
pause
```

## 使用 OADIN 命令行工具

构建完成后，您可以通过输入 `oadin -h` 来查看命令行工具的帮助信息。
...
```sh
# 将 AI 服务安装到本地
# OADIN 将安装必要的 AI 堆栈（如 ollama/openvino）和 OADIN 推荐的模型
oadin install chat
oadin install embed
...
规范请参见 [OADIN API 规范](https://intel.github.io/oadin/index.html).

值得注意的是，当前 OADIN 预览提供了基本的 chat 等服务，下一版本将会提供视频、音频相关的更多服务。
当前版本的文生图服务基于 OpenVINO 实现（仅支持 Windows 系统），通过 modelscope 拉取openvino转换过的 IR 格式的文生图模型提供服务。 

例如，您可以使用 `curl` 在 Windows 上测试聊天服务。

```sh
curl -X POST http://localhost:16688/oadin/v0.4/services/chat  -X POST -H
...
`http://localhost:16688/oadin/v0.4/api_flavors/openai/v1/chat/completions`。
...
`http://localhost:16688/oadin/v0.4/api_flavors/ollama/api/chat` 。同样，它位于 `api_flavors/ollama` ，
...
- OpenVINO模型引擎的接入
- 多模态模型的支持（本地及云端的文生图）
- Node SDK: 发布 Node.js 版 SDK（未来支持 .NET/Java/Swift等），提供比 OADIN Checker 更强的扩展性和可用性
- OADIN Checker 增强: 新增 .NET/C++/C/Python/Go/Node.js 多语言依赖包支持
- Demo Application: 新增基于 OADIN Checker 的集成示例项目
