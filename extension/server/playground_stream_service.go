...
func (p *PlaygroundImpl) SendMessageStream(ctx context.Context, request *dto.SendStreamMessageRequest) (chan *dto.ChatResponse, chan error) {
	...
	go func() {
		...
		var userMsg *entity.ChatMessage
		if request.Content != "" {
			// 添加当前用户消息
...
func (p *PlaygroundImpl) HandleToolCalls(ctx context.Context, sessionId string, messageId string) []map[string]string {
	messageQuery := &entity.ToolMessage{SessionID: sessionId, MessageId: messageId}
	messages, err := p.Ds.List(ctx, messageQuery, &datastore.ListOptions{
		SortBy: []datastore.SortOption{
...
// 更新工具调用的逻辑
func (p *PlaygroundImpl) UpdateToolCall(ctx context.Context, toolMessage *entity.ToolMessage) error {
