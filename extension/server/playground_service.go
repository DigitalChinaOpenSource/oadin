...
type Playground interface {
	...
	findRelevantContext(ctx context.Context, session *entity.ChatSession, query string) (string, error)
	findRelevantContextWithVSS(ctx context.Context, session *entity.ChatSession, query string, options RAGOptions) (string, error)
}
...
func (p *PlaygroundImpl) GetSessions(ctx context.Context) (*dto.GetSessionsResponse, error) {
	sessionQuery := &entity.ChatSession{}
...
func (p *PlaygroundImpl) GetMessages(ctx context.Context, request *dto.GetMessagesRequest) (*dto.GetMessagesResponse, error) {
	...
	for _, m := range messages {
		...
		var toolMessages []entity.ToolMessage
		if msg.IsToolGroupID {
			typeStr = "mcp"
			toolMsgQuery := new(entity.ToolMessage)
			...
			for _, tm := range toolMsgResults {
				if tmsg, ok := tm.(*entity.ToolMessage); ok {
...
func (p *PlaygroundImpl) findRelevantContextWithVSS(ctx context.Context, session *entity.ChatSession, query string, options RAGOptions) (string, error) {
	return "", nil
}
