...
因此，主版本号是访问 ``Oadin APIs`` URL 的一部分


Oadin 服务
====================

``Oadin Service`` 是一个提供人工智能功能的服务。它可以是一个聊天机器人、文本转语音服务、翻译服务等。每个 ``Oadin Service`` 都有它独特的 ``properties`` 和 ``API`` 。
...
每个 ``Oadin Service`` 都有一个独特的名称。该名称用于识别服务，并用于形成访问服务的端点 URL。

 ``Oadin Service`` 的典型名称包括 ``chat``, ``audio/text-to-speech``, ``embed`` 等。

``Oadin`` 截至目前支持 Restful 风格的 API。API 的 ``Endpoint`` 包括 HTTP 方法和访问服务的 URL。
URL 由主机名和端口（默认为 localhost 和 16688） Oadin 前缀、规范版本号以及服务名称组成，即 ``http://localhost:{port_number}/oadin/v{spec_version_number}/services/{service_name}``.

例如 ``chat`` 服务的 ``Endpoint`` 是 ``POST http://localhost:16688/oadin/v0.2/services/chat`` 。
...
Oadin 服务的元数据是一个描述服务的 JSON 对象。它可能因服务而异。然而，在服务的元数据中通常包含一些常见的字段。
...
.. list-table:: 
   ...
   * - local_service_providers
     - 用于引用： :ref:`Metadata of Oadin Service Provider`
     - 默认的本地服务提供商名称
...
Oadin 服务提供商实际上实施了相应的 Oadin 服务。

通常，对于一个给定的 Oadin 服务，它可能有多个服务提供商。例如，一个 ``/chat Oadin`` 服务可能由 ollama 在同一台机器上运行的本地 Oadin 服务提供商提供，同时还有一个由 OpenAI 提供的远程 Oadin 服务提供商。

每个 Oadin 服务提供商都有一个唯一的 ID。该 ID 用于识别服务提供商，并用于引用服务提供商，例如在 Oadin 服务的元数据中。参见： :ref:`Metadata of Oadin Service`.
...
Oadin 服务提供商的元数据是一个描述服务提供商的 JSON 对象。以下是大多数服务提供商都有的常见字段。
...
.. list-table:: 
   ...
   * - flavor
     - string, 例如, ``oadin``, ``openai``, ``ollama``, etc.
...
在 ``Oadin Service`` 的 Restful API 中，有一些常用的数据类型。例如，如何表示文本图像等。我们在这里描述这些通用类型。
...
OpenAI 还提供了 ```image_file``` ，它也在 Oadin 的未来路线图中。
...
服务通过 HTTP 调用，Oadin 服务要求请求和响应都使用 ``JSON`` 格式。
...
**NOTE** 请注意，如果基础实际服务提供商不支持可选字段，则在 Oadin 转换过程中将从请求中删除该字段。
...
响应中定义的必填字段始终由 Oadin 提供。

然而，返回响应中是否会出现可选字段，取决于处理相应请求的实际服务提供商。

与请求不同，Oadin 中未定义但由服务提供商返回的附加字段将仍然保留在那里，并直接发送到应用程序而不会被过滤掉。
...
.. list-table:: 
   ...
   * - 响应字段类型
     - 是否出现在底层服务提供商返回的响应中
     - 是否出现在 Oadin 返回的最终响应中
   * - 必需
     - | 未定。 
       | 底层服务提供商的响应可能包含相关信息，但未放入此字段。
     - | 会。 
       | Oadin 将转换并确保该字段被展示，并且该字段可安全访问
   * - 可选
     - 未定
     - | 未定。
       | Oadin 最终返回的响应仅包含由实际服务提供商提供的部分
...
**NOTE** 请注意，与调用云服务不同， ``Authorization`` 头不是始终必需的，因为它可能本地调用 Oadin 服务。然而，Oadin 可以根据配置或应用程序调用的信息稍后添加此类头。
...
.. list-table:: 
   ...
   * - model
     ...
     - 要用于此服务的模型。如未提供，将使用默认模型。如果服务不支持选择模型，则此字段将被忽略。参见 
       :ref:`Metadata of Oadin Service Provider` 中的 ``allow_to_select_model`` 。
       某些服务可能有多个模型，因此应用程序可能传递一个 JSON 对象来指定服务内部使用的每个模型的模型名称。
   * - stream
     - ``true`` 或 ``false``. 默认为 ``false``
     - 必需
     - 是否使用流模式。如未提供，将使用默认模式。参见： :ref:`Metadata of Oadin Service Provider` ``supported_response_mode``
   * - hybrid_policy
     - ``always_remote``, ``always_local``, ``default``
     - 可选
     - 混合策略使用。如未提供，将使用 ``default`` 策略。参见 :ref:`Metadata of Oadin Service` 中的 ``hybrid_policy``
   * - remote_service_provider
     - string, 或 JSON ，如 :ref:`Metadata of Oadin Service Provider`
     - 可选
     - 默认情况下，如果 Oadin 决定使用远程服务提供商来处理此调用（基于 hybrid_policy 等），它将使用平台定义的默认远程服务提供商为此服务提供服务。
       然而，应用程序可以通过使用此字段在请求中覆盖此特定设置，以确保当 Oadin 决定使用远程服务提供商时，使用该远程服务提供商。
       该字段需要是字符串，表示预定义服务提供商的 ID。或者，它也可以是一个 JSON 对象，表示 Oadin 服务提供商的： :ref:`Metadata of Oadin Service Provider`
   * - keep_alive
